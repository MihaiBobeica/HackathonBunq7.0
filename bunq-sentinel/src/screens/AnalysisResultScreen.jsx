...
export default function AnalysisResultScreen({ verdict, score, reasons, onStop, onContinue }) {
  ...
  const headline = verdict?.headline ?? 'Warden completed the scam check.';
  const reasonItems = (verdict?.reasons ?? reasons ?? []).map(reason => (
    typeof reason === 'string'
      ? { headline: reason, detail: '' }
      : reason
  ));
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={spring} className="absolute inset-0 bg-slate-50 p-6 z-40 overflow-y-auto no-scrollbar">
      <div className="rounded-[2rem] bg-white p-6 text-center soft-shadow-lg">
        ...
        <p className="text-sm font-black text-slate-700 mt-3">{headline}</p>
        {verdict?.confidence && (
          <p className="text-[10px] font-black text-slate-400 uppercase tracking-wide mt-1">Confidence: {verdict.confidence}</p>
        )}
      ...
      {safeActions.length > 0 && (
        <div className="mt-5 rounded-[2rem] bg-white p-5 soft-shadow space-y-3">
          <p className="text-[10px] font-black text-slate-400 uppercase tracking-wide">Suggested actions</p>
